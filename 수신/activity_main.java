 ...
 * 클래스 설명 : 이 클래스는 NFC태그 접촉시 넘어온 데이터를 second 액티비티로

 *                전송하는 역활을 한다.

 * 전역 변수 설명 : NfcAdapter nfcAdapter - 단말기의 NFC정보를 얻어온다.

 *                             PendingIntent pendingIntent - second 액티비티에 전송할 intent 값을 지닌다.

 *                             IntentFilter[] mlntentFilters - intent 필터지정

 *                             String[][] mNFCTechLists - 연결단말기 정보를 담는다

 *                             TextView text - 단말기의 NFC 상태를 알려주는 택스트 뷰
